...
ocd_demog <- mutate(ocd_demog, 
                    ...
                    gender = ifelse(K.nn == 'F', 'Female', 'Male'),
                    SubjID = MR4K.Nr,
                    .keep = 'none')
...
hc_demog <- select(hc_demog, c(MR4K.Nr, F.dt, K.nn)) %>%
  ...
  mutate(SubjID = MR4K.Nr, 
         birth_date = as.Date(F.dt, format = '%d.%m.%Y'),
         gender = ifelse(K.nn == 'J', 'Female', 'Male'),
         .keep = 'none')

demog <- rbind(kont_demog, ocd_demog)
...
# z-transform all columns representing variable brain volumes
  
  mutate(
    across(
      lh_bankssts_area : X5th.Ventricle,
      scale),
    .keep = 'unused') %>%
...
# pivot data to long format
  ...
  pivot_longer(
    cols = !c(SubjID, gender, age), 
    names_to = 'area_name', 
    values_to = 'mri')

write.csv(ocd_ytop, file = '~/mrs_data/mrs_lf_data.csv')
